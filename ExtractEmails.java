...
•	<host> is a sequence of at least two words, separated by dots '.'. Each word is sequence of letters
 and can have hyphens '-' between the letters.
...
Many users @ SoftUni confuse email addresses. We @ Softuni.BG provide high-quality training @
 home or @ class. –- steve.parker@softuni.de.	steve.parker@softuni.de
 * */
...
public class ExtractEmails {
	...
	public static void main(String[] args) {
		...
		String emailRegex = "(?<email>[a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\\.[a-zA-Z0-9_-]+)";	
		String input = scanner.nextLine();	
		List<String> emailText = new ArrayList<>();	
		Pattern mailPatternTxt = Pattern.compile(emailRegex);
		Matcher matcherEmail= mailPatternTxt.matcher(input);		
	    while (matcherEmail.find()){	    	
	    	String emailFinalText = matcherEmail.group("email");
	    	emailText.add(emailFinalText);
         }    
	    printEmailText(emailText);
	}
	 public static void printEmailText(List <String>emailText) {
	        for(String number :emailText){
